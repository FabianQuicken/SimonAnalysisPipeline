...
behavior = "left investigation"
behavior_net = "leftniffing"
...
for i in range(len(file_list_gt)):
    ...
    array_gt = df_gt[f"leftsniffing"].to_numpy()
    array_deg = df_deg[f"leftsniffing"].to_numpy()
...
ax0.set_title(f"{network} water investigation", color='white')
