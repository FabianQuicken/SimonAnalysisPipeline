...
path_deg = f"{project_path}/deg_predictions/*.csv"
...
for i in range(len(file_list_gt)):
    ...
    array_deg = df_deg["rightsniffing"].to_numpy()
...
ax0.set_title("DeepEthogram odor investigation", color='white')
...
ax1.set_title("DeepEthogram performance", color='white')
...
plt.tight_layout()
#plt.show()
plt.savefig(f"{project_path}/deg_ethograms_skip_stepsizetp{step_size_tp}_stepsize{step_size}_frames{frames}.svg", format='svg', facecolor="black")
